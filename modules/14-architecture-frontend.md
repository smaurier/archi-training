...
Qu'est-ce qu'un waterfall de requêtes et comment l'éviter ?|Des requêtes séquentielles alors qu'elles sont indépendantes (family → routines → streaks) : coût = somme des latences. On parallélise tout ce qui n'a pas de dépendance réelle (coût = max des latences) ; on n'enchaîne que les vraies dépendances.
